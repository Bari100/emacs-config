...
Most importantly the Emacs must be compiled with dynamic module support. Jinx requires libenchant, which is needed to compile the native module at install time.
If pkgconf or pkg-config is available, Jinx will use it to locate libenchant during installation. Depending on your BSD or Linux distribution you have to install different packages:
...
* Window Management
** Winner Mode
Turning on Winner mode. Then use winner-undo to switch back to an earlier window configuration saved by Winner mode
#+begin_src emacs-lisp
(winner-mode 1)
#+end_src
** Window Select

Automatically move cursor to help window to easily close it then.
#+begin_src emacs-lisp
